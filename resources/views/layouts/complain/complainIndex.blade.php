...
<style>
    ...
    <div class="container-fluid">
        ...
        <form action="{{ URL::asset('/complain/periode') }}" method="POST" id="complainForm">
            ...
            <div class="row">
                ...
                <div class="form-group ml-2">
                    <label>&nbsp;</label>
                    <form action="{{ URL::asset('/complain/downloadall') }}" method="POST" autocomplete="off">
                        @csrf
    ...
    // Fungsi untuk menggeser tombol action saat tabel di-scroll
    $(document).ready(function () {
        // Ambil semua tombol action berdasarkan kelas
        var actionButtons = $(".action-buttons");

        // Ambil posisi awal dari tombol action pertama
        var initialPosition = actionButtons.eq(0).offset().left;

        // Fungsi untuk menggeser tombol action berdasarkan posisi scroll
        function adjustActionButtons() {
            var scrollPosition = $(window).scrollLeft();
            actionButtons.each(function () {
                var button = $(this);
                var currentPos = initialPosition - scrollPosition;
                button.css("left", currentPos);
            });
        }

        // Panggil fungsi untuk mengatur posisi awal tombol action
        adjustActionButtons();

        // Panggil fungsi ketika tabel di-scroll atau halaman di-resize
