...
<style>
    ...
    .garis-bawah {
        border-top: 1px solid #000;
        margin-top: -1px; /* Menggeser garis ke atas untuk tumpang tindih dengan teks */
        position: relative; /* Diperlukan untuk posisi absolut pada anak .ttd */
    }
    /* Garis bawah */
    .garis-bawah-putus {
...
<body>
    <div class="container mt-4">
        ...
        <table class="mb-6">
            ...
            <tr>
                <td colspan="3">
                    <textarea class="form-control" id="detail" rows="18" width="100px">{{ $penyebabview }}</textarea>
                </td>
            </tr>
            <tr>
                <th class="th-small">Update Aplikasi :</th>
                <td class="td-small">:</td>
                <td>&nbsp</td>
            </tr>
            <tr>
                <td colspan="3">
                    <table border="1" class="tablee">
                        ...
                        @if(isset($aplikasi_data))
                        @foreach(json_decode($aplikasi_data) as $key => $aplikasi)
                        <tr>
                            <td>{{ $aplikasi->nama_aplikasi }}</td>
                            <td>{{ $aplikasi->ket_perubahan_aplikasi }}</td>
                            <td>{{ $aplikasi->tgl_solving }}</td>
                            <td>{{ $aplikasi->update_versi }}</td>
                            <td>{{ $aplikasi->tgl_live }}</td>
                        </tr>
                        @endforeach
                        @else
                        <tr>
